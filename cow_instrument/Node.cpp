...
Node::Node(Node const *const previous, Node_uptr &&next,
    ...
    : m_previous(previous), m_contents(contents), m_version(version) {
...
Node::Node(CowPtr<Component> contents, unsigned int version)
    : m_previous(nullptr), m_contents(contents), m_version(version) {}
...
Node_uptr Node::smartCopy(const Command &command, const Component &component,
  ...
  Node_uptr copy(new Node(newPrevious, m_contents, m_version + 1));
  if (this->hasChildren()) {
...
std::unique_ptr<Node> Node::clone() const {
    return clone(m_previous);
}

std::unique_ptr<Node> Node::clone(Node const * const previous) const
{
    Node_uptr copy(new Node(previous, m_contents, m_version));
    if (this->hasChildren()) {
      for (size_t i = 0; i < this->m_next.size(); ++i) {
