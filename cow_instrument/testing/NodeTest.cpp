...
TEST_F(NodeTest, test_construction_with_parent) {
  ...
  Node_sptr parent = std::make_shared<Node>(CowPtr<Component>(new MockComponent));
  CowPtr<Component> contents(new MockComponent);
...
TEST_F(NodeTest, test_construction_with_parent_and_child) {
  ...
  Node_sptr parent = std::make_shared<Node>(CowPtr<Component>(new MockComponent));
  Node_sptr child = std::make_shared<Node>(CowPtr<Component>(new MockComponent));
  CowPtr<Component> contents(new MockComponent);
...
TEST_F(NodeTest, test_modify){
    ...
    */
    ...
    EXPECT_NE(newTree->child()->child().get(), a->child()->child().get()); // compare c to c`

    // However, only the contents of one component should be different.

    EXPECT_EQ(&newTree->contents().const_ref(), &a->contents().const_ref());
    EXPECT_EQ(&newTree->child()->contents().const_ref(), &b->contents().const_ref());
    EXPECT_NE(&newTree->child()->child()->contents().const_ref(), &c->contents().const_ref());
